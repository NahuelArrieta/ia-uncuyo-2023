...
class Node:
    def __init__ (self, posX, posY, parent = None, cost = 0):
        self.posX = posX
        self.posY = posY
        self.parent = parent
        self.cost = cost
...
class Enviroment: 
    def __init__(self, sizeX, sizeY, obstacle_rate):
        ...
        self.end_posY = random.randint(0,sizeY-1)
        ...
        while obstacles>0:
            ...
            if self.chart[i][j] == 0 and (i!=self.init_posX or j!=self.init_posY) and (i!=self.end_posX or j!=self.end_posY):
                self.chart[i][j] = 1
                obstacles -= 1
    ...
    def is_end(self, node: Node):
        ...
        posX = node.posX
        posY = node.posY
        return (posX == self.end_posX and posY == self.end_posY)
    ...
    def right_node(self, node: Node):
        ...
        newNode.set_g(self.calculate_distance_to_end(newNode))
    ...
    def up_node(self, node):
        newNode = Node(node.posX-1, node.posY, node, node.cost+1)
        newNode.set_g(self.calculate_distance_to_end(newNode))
        if not(self.validate_pos(newNode)):
            return None
        return (newNode)
    ...
    def print_environment(self, path = None):
        ...
        ## Set end as 'X'
        print_chart[self.end_posX][self.end_posY] = "X"
