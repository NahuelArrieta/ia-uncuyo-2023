...
cross_validation <- function(dataframe, n) {
    ...
    for (i in 1:n) {
        ...
        train$especie <- factor(train$especie, levels = levels(dataframe$especie))
        train$ultima_modificacion <- factor(train$ultima_modificacion, levels = levels(dataframe$ultima_modificacion))
        train$altura <- factor(train$altura, levels = levels(dataframe$altura))
        train$diametro_tronco <- factor(train$diametro_tronco, levels = levels(dataframe$diametro_tronco))
        train$nombre_seccion <- factor(train$nombre_seccion, levels = levels(dataframe$nombre_seccion))
        ...
        test$especie <- factor(test$especie, levels = levels(dataframe$especie))
        test$ultima_modificacion <- factor(test$ultima_modificacion, levels = levels(dataframe$ultima_modificacion))
        test$altura <- factor(test$altura, levels = levels(dataframe$altura))
        test$diametro_tronco <- factor(test$diametro_tronco, levels = levels(dataframe$diametro_tronco))
        test$nombre_seccion <- factor(test$nombre_seccion, levels = levels(dataframe$nombre_seccion))
...
## Usage:
dataframe <- get_train_dataframe()
cross_validation(dataframe, 20)